...
cd("C:/Users/gealy/OneDrive/Documents/git/greatdsge/datacalc")
...
#productivity 
# Define parameters
rho = 0.75          # Coefficient for log(A_{t-1})
sigma_e = 0.05      # Standard deviation of e_t
normal_dist = Normal(0, sigma_e)  # Distribution for e_t

# Initialize the DataFrame (assuming mergeddf already exists)
n = nrow(mergeddf)  # Number of observations
mergeddf[!, :log_A] = zeros(n)  # Initialize log(A_t) column

# Set initial value for log(A_1) (e.g., log(A_1) = 0 for simplicity)
mergeddf[1, :log_A] = 0.0

# Simulate log(A_t) for t > 1
for t in 2:n
    mergeddf[t, :log_A] = rho * mergeddf[t-1, :log_A] + rand(normal_dist)
end

# Create a new column A_t centered at 1 (A_t = exp(log_A) + 1)
mergeddf[!, :A] = (mergeddf[!, :log_A]) .+ 1
...
# Create a lagged version of the :π column
mergeddf[!, :π_lag] = ShiftedArrays.lag(mergeddf.π, 1)

# Calculate the yearly growth rate (inflation rate) as a percentage change
mergeddf[!, :π_rate] = ((mergeddf.π .- mergeddf.π_lag) ./ mergeddf.π_lag) .* 100
...
start_date2 = Date("1993-01-01")
end_date2 = Date("2010-01-01")
...
# Create a new DataFrame with formatted min and max for numeric columns, and raw values for others
summary_table = DataFrame(
    Column = names(test),
    Minimum = [
        if eltype(test[:, col]) <: Number
            @sprintf("%.6f", minimum(test[:, col]))
        else
            minimum(test[:, col])  # Leave as-is for non-numeric columns
        end for col in names(test)
    ],
    Maximum = [
        if eltype(test[:, col]) <: Number
            @sprintf("%.6f", maximum(test[:, col]))
        else
            maximum(test[:, col])  # Leave as-is for non-numeric columns
        end for col in names(test)
    ]
)


show(summary_table, allrows = true)
